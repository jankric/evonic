"""
Exa web search tool — search the internet with neural or keyword queries.
"""

import os
import sys

# Ensure skill backend is importable
...
def _get_api_key() -> str:
    """Retrieve Exa API key from skill config."""
    try:
...
def _format_results(raw: dict) -> dict:
    """Format Exa API response into a clean agent-friendly structure."""
    results = []
...
def execute(agent: dict, args: dict) -> dict:
    ...
    if not api_key:
        return {
            'status': 'error',
            'message': 'Exa API key not configured. Ask the admin to set it in Web Search skill settings (Skills → Web Search → Settings).',
        }
